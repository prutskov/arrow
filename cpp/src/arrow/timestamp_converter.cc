...
namespace arrow {
FormattedTimestampConverter::FormattedTimestampConverter(const std::string& format)
    : format_(format) {}

int64_t ConvertTimePoint(const std::shared_ptr<DataType>& type,
                         arrow_vendored::date::sys_time<std::chrono::seconds> tp) {
...
bool FormattedTimestampConverter::operator()(const std::shared_ptr<DataType>& type,
...
std::unique_ptr<TimestampConverter> FormattedTimestampConverter::Make(
